...
help: ## Display this help message
	...
	@echo "  Build with specific profile using HYBRID_LIB_ADA_PROFILE variable:"
	@echo "    alr build                                        # standard (default)"
	@echo "    alr build -- -XHYBRID_LIB_ADA_PROFILE=embedded   # Ravenscar embedded"
	@echo "    alr build -- -XHYBRID_LIB_ADA_PROFILE=baremetal  # Zero footprint"
	@echo "    alr build -- -XHYBRID_LIB_ADA_PROFILE=concurrent # Multi-threaded"
	@echo "    alr build -- -XHYBRID_LIB_ADA_PROFILE=stm32h7s78"
	@echo "    alr build -- -XHYBRID_LIB_ADA_PROFILE=stm32mp135_linux"
	@echo ""
...
build-profiles: ## Validate library builds with all configuration profiles
	...
	@echo "$(CYAN)Uses: alr build -- -XHYBRID_LIB_ADA_PROFILE=<profile>$(NC)"
	@echo ""
	...
	for profile in $$PROFILES; do \
		...
		if [ -f "config/profiles/$$profile/$(PROJECT_NAME)_config.ads" ]; then \
			...
			if $(ALR) build --development -- -XHYBRID_LIB_ADA_PROFILE=$$profile -j8 2>&1 | grep -E 'error:|failed' > /dev/null; then \
...
submodule-update: ## Pull latest from all submodule repos
	...
	@echo "  git add docs scripts/python/shared test/scripts/python/shared"
